...
from services.registration_requester import RegistrationRequester
...
async def create_uploaders(no_clients: int, index_start: int = 1) -> list[FileUploader]:
    ...
    # register the connections before passing them along
    await register_connections(connections)
...
async def get_connections(no_clients: int, index_start: int) -> list[EvmConnection]:
    ...
    connection_tasks: list[Coroutine[None, None, EvmConnection]] = [
        EvmConnection.build_connection(Algorithm.ALGO3, index) for index in range(index_start, no_clients + index_start)
    ]

    return await asyncio.gather(*connection_tasks)

async def register_connections(connections: list[EvmConnection]) -> None:
    tasks: list[Coroutine[None, None, None]] = []
    for connection in connections:
        logging.info(f"Registering client: {connection.account.address}")
        registerer: RegistrationRequester = RegistrationRequester(connection)
