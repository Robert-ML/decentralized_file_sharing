...
class FileUploader:
    ...
    async def generate_upload_file(self) -> None:
        request_id: _FileRequestId = int.from_bytes(secrets.token_bytes(256 // 8), 'big', signed=False)
        logging.info(f"User \"{self.__connection.account.address}\" requesting to upload file, request_id: {request_id}")
        ...
        logging.info(f"User \"{self.__connection.account.address}\" finished uploading file, request_id: {request_id} | gas used: {total_gas_used}")
    ...
    async def __upload_file(self, file_creds: FilePrencCredentials) -> _GasUsed:
        ...
        message, formatted_plaintext, cyphertext = await asyncio.get_running_loop().run_in_executor(
            ...
            func=partial(__get_file_and_cyphertext, file_creds)
        )
...
def __get_file_and_cyphertext(file_creds: FilePrencCredentials) -> tuple[int, FQ12, Isshiki_Cyphertext_LV2]:
